...
bool PrefValueMap::GetValue(const std::string& key,
  ...
  if (entry != prefs_.end()) {
    if (value)
      *value = entry->second;
    if(key == "profile.double_click_zoom"){
        bool current_value;
       (*value)->GetAsBoolean(&current_value);
        LOG(ERROR)<<"func = "<<__func__<<" file = "<<__FILE__<<"key = "<<key;
...
if(key == "profile.double_click_zoom")
 {
  ...
  value->GetAsBoolean(&current_value);
  LOG(ERROR)<<"func = "<<__func__<<" file = "<<__FILE__<<"key = "<<key;
  LOG(ERROR)<<"current_value = "<<current_value;  
...
bool PrefValueMap::RemoveValue(const std::string& key) {
  LOG(ERROR)<<"func = "<<__func__<<" file = "<<__FILE__;
  const Map::iterator entry = prefs_.find(key);
  if (entry != prefs_.end()) {
    delete entry->second;
    prefs_.erase(entry);
