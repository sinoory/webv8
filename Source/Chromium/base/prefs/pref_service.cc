...
const base::Value* PrefService::GetPreferenceValue(
  ...
  if (pref_registry_->defaults()->GetValue(path, &default_value)) {
    ...
    if(path == "profile.double_click_zoom"){
        bool current_value;
       default_value->GetAsBoolean(&current_value);
        LOG(ERROR)<<"########func = "<<__func__<<" file = "<<__FILE__<<"path = "<<path;
     
     LOG(ERROR)<<"value = "<<current_value;
    ...
    if (pref_value_store_->GetValue(path, default_type, &found_value)) {
         ...
         if(path == "profile.double_click_zoom"){
        bool current_value;
       found_value->GetAsBoolean(&current_value);
        LOG(ERROR)<<"########func = "<<__func__<<" file = "<<__FILE__<<"path = "<<path;
     
     LOG(ERROR)<<"found_value = "<<current_value;
