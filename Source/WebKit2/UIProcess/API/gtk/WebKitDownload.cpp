...
enum {
    ...
    DOWNLOAD_FILENAME,
    DOWNLOAD_FILESIZE,
    DOWNLOAD_DATE_CREATED,
    DOWNLOAD_URL,
    DOWNLOAD_PATH
};

const std::string jsonKey[] = {"", "file-name", "file-size", "date-created", "url", "path"};
...
{ 
    ...
    if (!home.empty()) {
      config_file = home.Append(".config/download-lists");
    } else {
      std::string tmp_file = "/tmp/download-lists";
      base::FilePath tmp(tmp_file);
      config_file = tmp;
    }
    if(!download->priv->user_pref_store_)
        download->priv->user_pref_store_.reset(new JsonPrefStore(config_file));
    download->priv->download_list_lv = new base::ListValue();
    download->priv->pref_store = download->priv->user_pref_store_.get();
}

//Remove
void DeleteList(WebKitDownload* download, size_t index)
{
    download->priv->download_list_lv->Remove(index, NULL);
    download->priv->pref_store->SerializeAndWriteFile();
}

void SaveValueToDownloadList(WebKitDownload* download, 
                                                char *filename,
                                                char *filesize,
                                                char *datecreate,
                                                char *download_url,
...
{  
    ...
    if(!download->priv->download_list_lv)
    {
        download->priv->download_list_lv = new base::ListValue();
        download->priv->pref_store->SetValue("download-list", download->priv->download_list_lv);
    }

    /*
    base::DictionaryValue *dvcs= new base::DictionaryValue(); 
    size_t listsize = download->priv->download_list_lv->GetSize();
    g_printerr("listsize:%d\n", listsize);
    std::string filename_str;
    size_t i;
    for(i = 0; i < listsize; i++)
    {
        download->priv->download_list_lv->GetDictionary(i, &dvcs);
        dvcs->GetString(jsonKey[DOWNLOAD_FILENAME], &filename_str);
        g_printerr("filename_str:%s-%d\n", filename_str.data(), i);
        int cmp = strcmp(filename_str.data(), filename);
        g_printerr("cmp:%d\n", cmp);
        if(!cmp)
            {
    ...
    */
    ...
    base::DictionaryValue *dvc= new base::DictionaryValue(); 
    download->priv->download_list_lv->Append(dvc);
    strval_string = filename;
    dvc->Set(jsonKey[DOWNLOAD_FILENAME],new base::StringValue(strval_string));
    strval_string = filesize;
    dvc->Set(jsonKey[DOWNLOAD_FILESIZE],new base::StringValue(strval_string));
    strval_string = datecreate;
    dvc->Set(jsonKey[DOWNLOAD_DATE_CREATED],new base::StringValue(strval_string));
    strval_string = download_url;
    dvc->Set(jsonKey[DOWNLOAD_URL],new base::StringValue(strval_string));
    strval_string = path;
    dvc->Set(jsonKey[DOWNLOAD_PATH],new base::StringValue(strval_string));

    download->priv->pref_store->SerializeAndWriteFile();
}

gboolean SetDownloadList(WebKitDownload* download, 
                                                char **filename,
                                                char **filesize,
                                                char **datecreate,
                                                char **download_url,
...
{
    InitDownloadList(download);
    if(!download->priv->user_pref_store_->DoReading()) {
        g_printerr("reading no\n");
        download->priv->download_list_lv = NULL;
        return false;
    }
    else {
        g_printerr("reading ok\n");
        base::Value *vv = base::Value::CreateNullValue();
        download->priv->pref_store->GetValue("download-list", &vv);
        vv->GetAsList(&download->priv->download_list_lv);
        *listsize = download->priv->download_list_lv->GetSize();
    
        std::string filename_str;
        std::string filesize_str;
        std::string createtime_str;
        std::string url_str;
        std::string path_str;
        const base::DictionaryValue *dic_string = new base::DictionaryValue();
        size_t i;
        for(i = 0; i < *listsize; i++)
        {
            download->priv->download_list_lv->GetDictionary(i, &dic_string);
            dic_string->GetString(jsonKey[DOWNLOAD_FILENAME], &filename_str);
            dic_string->GetString(jsonKey[DOWNLOAD_FILESIZE], &filesize_str);
            dic_string->GetString(jsonKey[DOWNLOAD_DATE_CREATED], &createtime_str);
            dic_string->GetString(jsonKey[DOWNLOAD_URL], &url_str);
            dic_string->GetString(jsonKey[DOWNLOAD_PATH], &path_str);
            
            filename[i] = (char *)malloc((filename_str.length()+1)*sizeof(char));
            filesize[i] = (char *)malloc((filesize_str.length()+1)*sizeof(char));
            datecreate[i] = (char *)malloc((createtime_str.length()+1)*sizeof(char));
            download_url[i] = (char *)malloc((url_str.length()+1)*sizeof(char));
            path[i] = (char *)malloc((path_str.length()+1)*sizeof(char));
            
            memset(filename[i], 0x00, filename_str.length()+1);
            memset(filesize[i], 0x00, filesize_str.length()+1);
            memset(datecreate[i], 0x00, createtime_str.length()+1);
            memset(download_url[i], 0x00, url_str.length()+1);
            memset(path[i], 0x00, path_str.length()+1);
            
            //g_printerr("SetDownloadList strval_string:%s\n", filename_str.c_str());
            filename_str.copy(filename[i], filename_str.length(), 0);
            filesize_str.copy(filesize[i], filesize_str.length(), 0);
            createtime_str.copy(datecreate[i], createtime_str.length(), 0);
            url_str.copy(download_url[i], url_str.length(), 0);
            path_str.copy(path[i], path_str.length(), 0);
        }
