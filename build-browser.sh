...
#compile project on 64bits Linux machine
	...
	echo "build release version start..." && sleep 3
	cmake -DUSE_64BITS=1 -DPORT=GTK -DDEVELOPER_MODE=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_SKIP_BUILD_RPATH=FALSE -DCMAKE_BUILD_WITH_INSTALL_RPATH=FALSE -DCOMPILE_MODE=OFF -DENABLE_MIDORI=$BUILD_MIDORI && make VERBOSE=1 -j${CPU_NUM} && echo ******build release SUCCESS********
fi
	;;
"deb_package" )
